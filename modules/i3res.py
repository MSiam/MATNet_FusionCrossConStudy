...
""" Code from https://github.com/hassony2/inflated_convnets_pytorch """
...
from modules import inflate



class I3ResNet(torch.nn.Module):
    def __init__(self, resnet2d, frame_nb=16, class_nb=1000, conv_class=False, center=True):
        """
        Args:
        ...
        self.layer1 = inflate_reslayer(resnet2d.layer1, center=center)
        self.layer2 = inflate_reslayer(resnet2d.layer2, center=center)
        self.layer3 = inflate_reslayer(resnet2d.layer3, center=center)
        self.layer4 = inflate_reslayer(resnet2d.layer4, center=center)
...
def inflate_reslayer(reslayer2d, center=True):
    reslayers3d = []
    for layer2d in reslayer2d:
        layer3d = Bottleneck3d(layer2d, center=center)
        reslayers3d.append(layer3d)
    return torch.nn.Sequential(*reslayers3d)


class Bottleneck3d(torch.nn.Module):
    def __init__(self, bottleneck2d, center=True):
        ...
        if bottleneck2d.downsample is not None:
            self.downsample = inflate_downsample(
                bottleneck2d.downsample, time_stride=spatial_stride, center=center)
        else:
...
def inflate_downsample(downsample2d, time_stride=1, center=True):
